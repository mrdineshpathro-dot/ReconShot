"""
Standalone Dark-Themed HTML Report Generator for ReconShot.
...
def generate_html_report(summary: ScanSummary, report_path: Path) -> Path:
    """
    Generate an ultra-modern, dark-themed, standalone HTML report gallery.
    """
    report_path.parent.mkdir(parents=True, exist_ok=True)

    # Compute status code breakdown
    ...
    for idx, r in enumerate(summary.results):
        ...
        # Relative path from report.html (inside reports/) to screenshots/
        ...
        if r.screenshot:
            card_img_html = f"""
            <div class="card-image-wrapper" onclick="openLightbox('{screenshot_rel}', '{escape(r.url)}', '{status_text}')">
                <img src="{screenshot_rel}" alt="Screenshot of {escape(r.url)}" loading="lazy" class="card-img" />
                <div class="image-overlay"><span class="overlay-icon">🔍 View Fullscreen</span></div>
            </div>
            """
        else:
        ...
        headers_json_str = escape(json.dumps(r.headers_received or {}, indent=2))

        cards_html.append(f"""
        <div class="recon-card" data-idx="{idx}" data-category="{status_category}" data-search="{escape(r.url.lower())} {escape(r.page_title.lower() if r.page_title else '')} {status_text}">
            {card_img_html}
            <div class="card-body">
                <div class="card-header-row">
                    <span class="badge {status_cls}">{status_text}</span>
                    <span class="resp-tag">⏱ {resp_time_disp}</span>
                    <span class="vp-tag">🖥 {vp_disp}</span>
                </div>
                ...
                <details class="card-details">
                    ...
                    <div class="details-content">
                        ...
                        <div><strong>Redirects:</strong> <code>{r.redirect_count}</code></div>
                        {f'<div><strong>Content-Type:</strong> <code>{escape(r.content_type)}</code></div>' if r.content_type else ''}
                        {f'<div><strong>Headers:</strong><pre>{headers_json_str}</pre></div>' if r.headers_received else ''}
                        {f'<div><strong>Error:</strong> <span class="err-text">{escape(r.error)}</span></div>' if r.error else ''}
                    </div>
                </details>
            </div>
        </div>
        ...
        # Table row HTML
        thumb_html = f'<img src="{screenshot_rel}" class="table-thumb" onclick="openLightbox(\'{screenshot_rel}\', \'{escape(r.url)}\', \'{status_text}\')">' if r.screenshot else '<span class="dim-text">None</span>'
        table_rows_html.append(f"""
        <tr data-category="{status_category}" data-search="{escape(r.url.lower())} {escape(r.page_title.lower() if r.page_title else '')} {status_text}">
            <td>{thumb_html}</td>
...
<head>
    ...
    <title>ReconShot Report — Automated Web Reconnaissance</title>
    <style>
        :root {{
            --bg-primary: #090d16;
            --bg-secondary: #0f172a;
        ...
        * {{
        ...
        /* Header Bar */
        .header {{
            background: linear-gradient(180deg, #111a2e 0%, var(--bg-primary) 100%);
        ...
        .no-img-icon {{ font-size: 32px; margin-bottom: 8px; }}
        .no-img-text {{ font-size: 13px; color: var(--accent-red); word-break: break-word; }}
        ...
        .status-3xx {{ background: rgba(79, 172, 254, 0.15); color: var(--accent-blue); border: 1px solid rgba(79, 172, 254, 0.3); }}
        ...
        .details-content code, .details-content pre {{
            background: #060911;
            padding: 4px 6px;
            border-radius: 4px;
...
<body>
    ...
    <header class="header">
        <div class="header-content">
            <div>
                ...
                <div class="brand-subtitle">Automated Web Application Screenshot Reconnaissance</div>
            </div>
    ...
    <main class="container">
        ...
        <section class="stats-grid">
            ...
            <div class="stat-card">
                ...
                <div class="stat-value stat-val-green">{summary.successful}</div>
            </div>
            <div class="stat-card">
                <div class="stat-label">Failed / Offline</div>
                <div class="stat-value stat-val-red">{summary.failed}</div>
            </div>
            <div class="stat-card">
                <div class="stat-label">2xx Success</div>
                <div class="stat-value stat-val-green">{c_2xx}</div>
            </div>
            <div class="stat-card">
                <div class="stat-label">3xx Redirects</div>
                <div class="stat-value stat-val-blue">{c_3xx}</div>
            </div>
            <div class="stat-card">
                <div class="stat-label">4xx Client Errors</div>
                <div class="stat-value stat-val-yellow">{c_4xx}</div>
            </div>
            <div class="stat-card">
                <div class="stat-label">5xx Server Errors</div>
                <div class="stat-value stat-val-red">{c_5xx}</div>
            </div>
            <div class="stat-card">
                <div class="stat-label">Avg Response</div>
                <div class="stat-value stat-val-cyan">{avg_time} ms</div>
            </div>
            <div class="stat-card">
                <div class="stat-label">Scan Duration</div>
                <div class="stat-value">{summary.duration_seconds:.1f}s</div>
            </div>
        </section>
        ...
        <section class="controls-panel">
            <div class="search-box">
                ...
                <input type="text" id="searchInput" class="search-input" placeholder="Search target URL, title, or status..." oninput="filterResults()">
            </div>
    ...
    <script>
        ...
        function openLightbox(src, url, status) {{
            const modal = document.getElementById('lightboxModal');
            const modalImg = document.getElementById('modalImg');
            const modalTitle = document.getElementById('modalTitle');

            modalImg.src = src;
            modalTitle.innerHTML = `<span class="badge status-2xx">${{status}}</span> <strong>${{url}}</strong>`;
        ...
        function exportJSON() {{
            ...
            dlAnchorElem.setAttribute("download", "reconshot_results.json");
            dlAnchorElem.click();
        }}
    ...
    logger.debug(f"Generated HTML report at {report_path}")
    return report_path
